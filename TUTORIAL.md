...
5. **自動連線**：勾選後立即對所有**離線**網卡指派候選節點並連線；之後每隔一段時間自動補上離線網卡，被踢或斷線時直接改派其他候選節點（不會連回原伺服器）。取消勾選則只保留目前已建立的連線，不再自動重連。
> **進階設定**：建立連線設定時，程式會以 `vpncmd` 自動套用 SoftEther 的進階選項——TCP 連線數 3、斷開後不自動重連（改由本程式換節點）。這些對應 VPN Client 的「高級設置」，不需手動開啟 GUI 調整。
